    ...
    <div class="col-md-4 offset-md-1">
        ...
        <div>
            <div class="row gx-3 gy-2">
                ...
                <div class="col-md-12" v-for="item in useArmorStore().material_list" :key="item.id" v-show="item.slot <= useArmorStore().slot_calc[useArmorStore().material_type.find(a => a.name === item.type)?.word ?? '']">
                    <div class="materials-list" @click="useArmorStore().selectItem(item)">
                        <span class="checkbox checkbox-click">
                            ...
                            <div class="list-right">
                                <img :src="`/img/attr/${useArmorStore().material_type.find(a => a.name === item.type)?.icon || ''}.png`">
                                <span>[{{" " + item.slot + " " }}]</span>
                                <img v-for="attr in item.attribute" :src="`/img/attr/${useArmorStore().material_attribute.find(a => a.name === attr)?.icon || ''}.png`">
                            </div>
                        </span>
                    </div>
                </div>
                <div class="col-md-12">
                    <hr>
                    <div class="checkbox checkbox-click" v-show="useArmorStore().material_list == null || useArmorStore().material_list.length == 0">
                        <span>暂无可以选择的材料哦~</span>
                    </div>
                    <div class="checkbox checkbox-click" v-show="useArmorStore().material_list != null && useArmorStore().material_list.length != 0 && !useArmorStore().hasValidMaterial">
                        <span>槽位不足~</span>
                    </div>
                </div>
            </div>
